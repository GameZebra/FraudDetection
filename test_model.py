...
on real credit card transaction data with visualizations
"""
...
# Load the data
DATA_FILE = Path(__file__).parent / "data" / "creditcard.csv"
print("="*70)
...
def create_confusion_matrix_plot(y_true, y_pred, title="Confusion Matrix"):
    ...
    # Add custom text with TP, FP, TN, FN labels
...
def create_pr_curve(y_true, y_pred_proba, title="Precision-Recall Curve"):
    ...
    ax.plot(recall, precision, color='green', lw=3, label=f'PR curve (AUC = {pr_auc:.4f})')
...
# Load metadata to check the best model
...
# Split into fraud and legitimate for testing
...
# Test 1: Sample of fraudulent transactions
print(f"\n" + "="*70)
print("TEST 1: PREDICTING ON FRAUDULENT TRANSACTIONS")
print("="*70)

if len(fraud_transactions) > 0:
    # Take first 10 fraud cases
...
# Test 3: High-risk and low-risk transactions
print(f"\n" + "="*70)
print("TEST 3: ANALYZING FRAUD PROBABILITY DISTRIBUTION")
print("="*70)

# Get predictions for all data to analyze distribution
print("\nPredicting on all transactions (this may take a moment)...")
...
# Test 4: Confusion matrix on full test set
print(f"\n" + "="*70)
